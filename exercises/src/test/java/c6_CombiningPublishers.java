...
public class c6_CombiningPublishers extends CombiningPublishersBase {
     ...
     *
     * Answer:
     * - Is there a difference between Mono.flatMap() and Flux.flatMap()? - returning type Mono/Flux
     */
    @Test
     ...
     *
     * Answer:
     * - What is difference between concatMap() and flatMap()? concatMap preserves order
     * - What is difference between concatMap() and flatMapSequential()?
     *   flatMapSequential queues elements, concatMap waits for element completion before going to next one
     * - Why doesn't Mono have concatMap() operator? concatMap need multiple values (to concatenate) to be useful
     */
    @Test
    ...
    public void acid_durability() {
        ...
        Flux<String> committedTasksIds = tasksToExecute().flatMapSequential(stringMono ->
                stringMono.doOnSuccess(this::commitTask)
        );
