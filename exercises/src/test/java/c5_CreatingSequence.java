...
public class c5_CreatingSequence {
    ...
    public void generate_programmatically() {
        ...
        var generateCounter = new AtomicInteger(0);
        Flux<Integer> generateFlux = Flux.generate(sink -> {
            if (generateCounter.get() > 5) {
                sink.complete();
            }
            sink.next(generateCounter.getAndIncrement());
        });

        //------------------------------------------------------

        var createCounter = new AtomicInteger(0);
        Flux<Integer> createFlux = Flux.create(sink -> {
            while(createCounter.get() <= 5) {
                sink.next(createCounter.getAndIncrement());
            }
            sink.complete();
        });
        ...
        var pushCounter = new AtomicInteger(0);
        Flux<Integer> pushFlux = Flux.push(sink -> {
            while(pushCounter.get() <= 5) {
                sink.next(pushCounter.getAndIncrement());
            }
            sink.complete();
        });
